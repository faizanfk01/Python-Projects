...
if code_decode == "code":
    ...
    for word in words:
        if len(word) >= 3:
            ...
            coded_words.append(word[::-1])  # reverse short word
    ...
    for word in words:
        if len(word) > 6:  # must be long enough to trim 3 from start/end
            trimmed = word[3:-3]
            if trimmed:  # check it's not empty
                decoded = trimmed[-1] + trimmed[:-1]
            else:
                decoded = ""  # fallback
            decoded_words.append(decoded)
        else:
            decoded_words.append(word[::-1])  # reverse short/invalid words
